...
def dump_ngrams(ngrams):
    if not os.path.exists(config.DIR_DUMP):
        os.makedirs(config.DIR_DUMP)

    dump_file = os.path.join(config.DIR_DUMP, 'dump_edges.pkl')
    with open(dump_file, 'wb') as f:
        cPickle.dump(ngrams, f)
